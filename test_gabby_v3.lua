#!/usr/bin/env lua
-- test_gabby_v3.lua
-- Advanced continuous test script for GabbyLua P2P chat

local socket = require("socket")
local json = require("cjson")
local colors = require("ansicolors")

-- Load GabbyLua modules
...
local TEST_CONFIG = {
    ...
    UDP_TEST_INTERVAL = 7,
    DISPLAY_REFRESH = 0.5, -- Faster refresh
    PEER_TIMEOUT = 30,
    UDP_DISCOVERY_PORT = config.UDP_PORT or 9001,
    TCP_MESSAGE_PORT = config.TCP_PORT or 9002,
    UDP_MESSAGE_PORT = 9003,
    MAX_MESSAGE_LOG = 15, -- More messages
    MAX_PEERS_DISPLAY = 7,
    TCP_TIMEOUT = 1.5, -- Shorter timeout for faster testing
    UDP_TEST_SIZE = 512 -- Larger test payload
}

-- Enhanced test runner state
local TestRunner = {
    start_time = os.time(),
    ...
    stats = {
        ...
        tcp = {sent = 0, received = 0, success = 0, errors = 0, total_rtt = 0, min_rtt = 9999, max_rtt = 0},
        udp = {sent = 0, received = 0, success = 0, errors = 0, total_rtt = 0, min_rtt = 9999, max_rtt = 0}
    },
    message_log = {},
    running = true,
    test_sequence = 0,
...
-- Advanced colorization with fallback
...
local function update_peer(ip, port, peer_data)
    ...
    if TestRunner.peer_lock[peer_id] and TestRunner.peer_lock[peer_id] > os.time() - 0.5 then
...
-- Advanced TCP testing with RTT tracking
local function send_tcp_test(peer_id, peer)
    ...
    local success, err = xpcall(function()
        tcp_socket:settimeout(TEST_CONFIG.TCP_TIMEOUT)
    ...
    if success then
        ...
        TestRunner.stats.tcp.min_rtt = math.min(TestRunner.stats.tcp.min_rtt, rtt)
        TestRunner.stats.tcp.max_rtt = math.max(TestRunner.stats.tcp.max_rtt, rtt)
        
        -- Track per-peer RTT
        if not peer.rtt then peer.rtt = {} end
        table.insert(peer.rtt, rtt)
        if #peer.rtt > 10 then table.remove(peer.rtt, 1) end
...
-- UDP testing with payload verification
local function send_udp_test(peer_id, peer)
    TestRunner.test_sequence = TestRunner.test_sequence + 1
    local test_payload = string.rep("X", TEST_CONFIG.UDP_TEST_SIZE)
    local test_msg = {
        type = "udp_test",
        payload = test_payload,
        size = TEST_CONFIG.UDP_TEST_SIZE,
    ...
    if sent then
        ...
        TestRunner.stats.udp.min_rtt = math.min(TestRunner.stats.udp.min_rtt, rtt)
        TestRunner.stats.udp.max_rtt = math.max(TestRunner.stats.udp.max_rtt, rtt)
        log_message("udp", "sent", peer_id, "UDP test message (" .. TEST_CONFIG.UDP_TEST_SIZE .. " bytes)", rtt)
    else
        TestRunner.stats.udp.sent = TestRunner.stats.udp.sent + 1
...
local function listen_udp_messages()
    ...
    if msg then
        ...
        if ok and test_data.type == "udp_test" then
            log_message("udp", "received", ip .. ":" .. port, "UDP test message (" .. #test_data.payload .. " bytes)", nil)
        end
    end
end
...
local function draw_peers()
    ...
    for peer_id, peer in pairs(TestRunner.peers) do
        ...
        if peer.status == "online" and peer.rtt and #peer.rtt > 0 then
            local avg_rtt = 0
            for _, rtt in ipairs(peer.rtt) do avg_rtt = avg_rtt + rtt end
            avg_rtt = math.floor(avg_rtt / #peer.rtt)
            rtt_str = string.format(" | RTT: %dms", avg_rtt)
        end
...
-- Main test loop with performance monitoring
...
local function run_continuous_tests()
    ...
    local tick_start = socket.gettime()
    ...
    local tick_time = socket.gettime() - tick_start
    if tick_time > 0.1 then
        log_message("warning", "system", "localhost", 
            string.format("High tick time: %.3fs - consider reducing test frequency", tick_time), nil)
    end
end
...
local function main()
    ...
    while TestRunner.running do
        ...
        if not ok then
            ...
            local sleep_time = TEST_CONFIG.DISPLAY_REFRESH - (socket.gettime() - TestRunner.last_tick)
            if sleep_time > 0 then
                socket.sleep(sleep_time)
            end
        end
        TestRunner.last_tick = socket.gettime()
    end
end
